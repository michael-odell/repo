...
var commands = []command{
	...
	{name: "sync", summary: "reconcile repos toward the registry", run: cmdSync,
		help: "usage: repo sync [flags] [root|path|repo...]\n\nReconcile the selected repos toward the registry. With no selector, all repos\nare included; a selector may be a root name, a directory path, or a repo name.\n\nflags:\n  --if-due        only sync repos whose cadence is due\n  --force         ignore cadence\n  --fix           apply the config↔disk reconciliations sync reports\n  --lose-ignored  with --fix, discard .gitignore'd files without prompting\n  --no-prune      don't act on landed branches this run; still report them\n  -n, --dry-run   show planned actions without changing anything\n  -v, --verbose   explain the decision for every repo"},
	{name: "prune", summary: "delete local branches whose work has landed", run: cmdPrune,
		help: "usage: repo prune [flags] [root|path|repo...]\n\nClassify each repo's task branches against its important branches and delete the\nones that have fully landed — including via squash or rebase, which `git branch\n--merged` cannot see.\n\nOn a terminal every candidate is walked one at a time, evidence first: y deletes,\nn keeps, a says yes to the rest of that repo, q stops. With no terminal there is\nnobody to ask, so nothing is deleted unless --yes says so.\n\nEvery deletion is recorded — repo, branch, the SHA it held, and the command that\nputs it back — under $XDG_STATE_HOME/repo/prune.log.\n\nflags:\n  --yes          delete without asking (required when there is no terminal)\n  --dry-run      run every check, delete nothing, and print what would go\n  -v, --verbose  print the evidence behind every verdict"},
